            ...
            <div className="my-5">
                {
                    isLoading ?
                        ...
                        <div className="flex flex-col justify-center items-center">
                            ...
                            <SubmitBtn user={userInfo} stuBatch={stuBath} stuData={stuData}></SubmitBtn>
...
const SubmitBtn = ({ user, stuBatch, stuData }) => {

    const stuBath = useParams().class;
    const handleReset = () => {
        Swal.fire({ title: `Are you sure?`, text: `If you confirm All present will be clear`, showDenyButton: true, showCancelButton: true, confirmButtonText: 'Confirm', denyButtonText: `Not Confirm`, }).then((result) => {
    ...
    const handleSubmit = () => {
        ...
        if (dataFromSessionStor === null || dataFromSessionStor.length === 0) {
            toast.error('You dont call any student', { duration: 1000, style: { border: '2px solid #ff3200', padding: '8px', color: '#000000', }, iconTheme: { primary: '#ff2222', secondary: '#ffffff', }, })
        } else {
            Swal.fire({ text: `Total Present: ${dataFromSessionStor.length}, out of ${stuData.length}, and absent student is ${stuData.length - dataFromSessionStor.length}`, title: `Do you want to submit the Attendence of ${batchToClass[stuBath]}`, showDenyButton: true, showCancelButton: true, confirmButtonText: 'Confirm', denyButtonText: `Not Confirm`, }).then((result) => {
                /* Read more about isConfirmed, isDenied below */
                if (result.isConfirmed) {
                    const presentIds = [];
                    const absentIds = [];
                    for (const i of stuData) {
                        dataFromSessionStor.includes(i._id) ? presentIds.push(i._id) : absentIds.push(i._id);
                    }
                    const presentedList = [presentIds, absentIds];
                    const userUid = user.uid;
                    console.log(user);
                    const fullData = { userUid, stuBatch, presentedList };
                    fetch(`https://school-student-info-client.vercel.app/students_present`, { method: "POST", headers: { 'content-type': 'application/json' }, body: JSON.stringify(fullData) }).then(res => res.json()).then(data => console.log(data));
